...
TabSIR <- fluidRow( box(title = "SIR", width = 8,shinyjs::useShinyjs(),
    sliderInput("infectadosSIR", "Infectados:", 1, 1000, 1, step = 1),
    sliderInput("suceptiblesSIR", "Suceptibles:", 1, 1000, 1, step = 1),
...
tabBox(title = "Metodos",width = 8,tabPanel("Adams",width = 400,
...
CalcularSIR <- function(){
  output$plot1SIR <- renderPlot({
    N = 1
    init <- c(S = input$suceptiblesSIR,I = input$infectadosSIR,R = 0)
    ...
    attach(out)
    plot(out$time, out$S, type="l", col="red", ylim=c(0,800), xlab="Dias", ylab="Infectados",main = "Runge Kutta 4")
    par(new=T)
    plot(out$time, out$I ,col="blue", type="l", xlab="Dias", ylab="Infectados", ylim = c(0,800))
    par(new=T)
    plot(out$time, out$R ,type = "l", col="black", xlab="Dias", ylab="Infectados", ylim = c(0, 800))
  ...
  output$plot2SIR <- renderPlot({
    ...
    out <- as.data.frame(out*N)
    attach(out)
    plot(out$time, out$S, type="l", col="red", ylim=c(0,800), xlab="Dias", ylab="Infectados",main = "Adams")
    par(new=T)
    plot(out$time, out$I ,col="black", type="l", xlab="Dias", ylab="Infectados", ylim = c(0,800))
    par(new=T)
    plot(out$time, out$R ,type = "l", col="purple", xlab="Dias", ylab="Infectados", ylim = c(0, 800))
    legend(x = "topright", legend=c("Susceptibles", "Infectados", "Recuperados"), col=c("red", "black", "purple"), lty=rep(1, 2))
  })
